...
function _qkaspq:Init(titleText)
	...
	local NOTIFY_SOUNDS = {
		["Стандартный"] = "rbxassetid://103421304020039",
		["Телеграм"] = "rbxassetid://91271439961236",
		["Айфон"] = "rbxassetid://93707490272210",
		["Разное"] = "rbxassetid://225320558",
		["Виндовс 11"] = "rbxassetid://82130248702656",
	}
	getNotifySettings = function()
		...
		if not notifyMod then
			return true, 3, "rbxassetid://103421304020039", 0.5, "Справа"
		end
		local enabled = notifyMod.on
		local duration = 3
		local soundId = "rbxassetid://103421304020039"
		local volume = 0.5
		local location = "Справа"
		for _, opt in ipairs(notifyMod.opts or {}) do
			if opt.label == "Длительность" then
				duration = opt.value
			elseif opt.label == "Звук Уведомления" then
				soundId = NOTIFY_SOUNDS[opt.value] or "rbxassetid://103421304020039"
			elseif opt.label == "Громкость" then
				volume = opt.value / 100
			elseif opt.label == "Расположение" then
				location = opt.value
			end
		end
	...
	function self:Notify(text, icon)
		...
		task.spawn(function()
			...
			if isCursor then
				...
				if location == "Справа" then
					notifyContainer.Position = UDim2.new(1, -20, 1, -80)
					notifyContainer.AnchorPoint = Vector2.new(1, 1)
					notifyListLay.VerticalAlignment = Enum.VerticalAlignment.Bottom
					notifyListLay.HorizontalAlignment = Enum.HorizontalAlignment.Right
				elseif location == "Слева" then
	...
	local function makeColorPicker(parent, data, updateHeight)
		...
		data.ui_checkIcon = checkIcon
		local label = Instance.new("TextLabel")
		label.Size = UDim2.new(1, -64, 0, 14)
		...
		box.Parent = frame
		rnd(box, 4)
		stk(box, Color3.fromRGB(45, 45, 52), 1)
		data.ui_box = box
		...
		local function updateColor()
			...
			if data.callback then
				pcall(data.callback, col, data.value)
			end
		end
		...
		checkBtn.Size = UDim2.new(1, -20, 0, 22)
		...
		checkBtn.MouseButton1Click:Connect(function()
			data.value = not data.value
			tw(checkbox, {BackgroundColor3 = data.value and ac or cl.check}, 0.15)
			tw(checkIcon, {ImageTransparency = data.value and 0 or 1}, 0.15)
			if data.callback then
				pcall(data.callback, data.color or Color3.fromRGB(120, 110, 250), data.value)
			end
			triggerBindRefresh()
		end)
		local btn = Instance.new("TextButton")
	...
	local function makeButton(parent, data, updateHeight)
		...
		btn.Parent = frame
		rnd(btn, 5)
		stk(btn, Color3.fromRGB(36, 36, 42))
		data.ui_btn = btn
		data.originalLabel = data.originalLabel or data.label
		btn.MouseEnter:Connect(function()
	...
	local function makeText(parent, data, updateHeight)
		...
		lbl.TextSize = 11
		lbl.Font = Enum.Font.MontserratMedium
		lbl.TextXAlignment = Enum.TextXAlignment.Left
		lbl.RichText = true
		lbl.Parent = frame
		data.ui_label = lbl
		...
		if data.desc then
			...
			sub.TextSize = 10
			sub.Font = Enum.Font.MontserratMedium
	...
	local function makeTextBox(parent, data, updateHeight)
		...
		box.TextSize = 11
		box.Font = Enum.Font.MontserratMedium
	...
	local function buildCard(modData, colIdx, order)
		...
		togBtn.MouseButton1Click:Connect(function()
			...
			triggerBindRefresh()
			self:Notify("Модуль <font color=\"rgb(255, 255, 255)\">" .. modData.name .. "</font> " .. (modData.on and "<font color=\"rgb(80, 220, 100)\">включен</font>" or "<font color=\"rgb(240, 80, 80)\">выключен</font>"), MOD_ICON)
...
_qkaspq:CreateModule("Settings", {
	...
	bind = "None",
	nobind = true,
	desc = "Настройка всплывающих уведомлений.",
	opts = {
		{type = "slider", label = "Длительность", value = 3, min = 1, max = 10, suffix = " сек"},
		{type = "slider", label = "Громкость", value = 50, min = 0, max = 100, suffix = "%"},
		{type = "dropdown", label = "Звук", value = "Стандартный", list = {"Стандартный", "Телеграм", "Айфон", "Разное", "Виндовс 11"}},
		{type = "dropdown", label = "Расположение", value = "Справа", list = {"Справа", "Слева", "Центр", "Курсор"}},
		{type = "button", label = "Тест звука", callback = function()
...
_qkaspq:CreateModule("Settings", {
	...
	bind = "None",
	nobind = true,
	desc = "Показывать окно с горячими клавишами.",
...
_qkaspq:CreateModule("Settings", {
	...
	bind = "None",
	nobind = true,
	desc = "Список включённых модулей на экране.",
	opts = {
		{type = "dropdown", label = "Расположение", value = "Справа", list = {"Слева", "Справа"}, callback = function()
